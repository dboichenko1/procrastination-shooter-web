...
function buildSvg() {
  ...
  // анатомическая фигурка Сизифа — суставы пересчитываются покадрово (см. figureJoints).
  // Дальняя нога приглушена для глубины; конечности толще для «телесного» силуэта.
  function limb(w, op) { return sel('line', { stroke: C.ink, 'stroke-width': w, 'stroke-linecap': 'round', opacity: op || 1 }); }
  els.fig = {
    thighR: limb(4, 0.6), shinR: limb(4, 0.6),
    thighL: limb(4.6), shinL: limb(4.6),
    torso: limb(5.6), armU: limb(4, 0.92), armF: limb(4, 0.92),
    head: sel('circle', { r: 6, fill: C.ink }),
  };
  var figG = sel('g', {});
  appendKids(figG, [els.fig.thighR, els.fig.shinR, els.fig.thighL, els.fig.shinL, els.fig.torso, els.fig.armU, els.fig.armF, els.fig.head]);
  svg.appendChild(figG);

  // крупный валун (с трещинами, чтобы видеть качение). Рисуется поверх — ладони уходят за него.
  els.boulder = sel('g', {});
  els.boulder.appendChild(sel('circle', { cx: 0, cy: 0, r: BOULDER_R, fill: C.stone, stroke: C.ink, 'stroke-width': 1.6 }));
  els.boulder.appendChild(sel('path', { d: 'M-14,-9 L-2,3 L9,-13', fill: 'none', stroke: C.ink, 'stroke-width': 1.3, opacity: 0.4 }));
  els.boulder.appendChild(sel('path', { d: 'M5,10 L15,3', fill: 'none', stroke: C.ink, 'stroke-width': 1, opacity: 0.28 }));
...
function renderScene() {
  ...
  var bc = vadd(surfPt(t), vmul(VN, BOULDER_R));
  // качение без проскальзывания: угол поворота = пройденный путь / радиус
  var spin = (running && !reduceMotion) ? (t * SLEN / BOULDER_R) * 180 / Math.PI : 0;
  els.boulder.setAttribute('transform', 'translate(' + bc[0] + ',' + bc[1] + ') rotate(' + spin + ')');

  // анатомическая фигурка: режим позы + фаза шага
  var mode = !running ? 'idle' : (pos.phase === 'rollback' ? 'fall' : 'push');
  var wp = (running && !reduceMotion && mode === 'push') ? ((elapsed % STRIDE_MS) / STRIDE_MS) : 0;
  var J = figureJoints(bc, t, wp, mode);
  setLine(els.fig.thighR, J.hip, J.kneeR); setLine(els.fig.shinR, J.kneeR, J.footR);
  setLine(els.fig.thighL, J.hip, J.kneeL); setLine(els.fig.shinL, J.kneeL, J.footL);
  setLine(els.fig.torso, J.hip, J.sh);
  setLine(els.fig.armU, J.sh, J.elbow); setLine(els.fig.armF, J.elbow, J.hand);
  setDot(els.fig.head, J.head);
